...
def load_images(npz_path, max_samples=100):
    ...
    tensor = torch.tensor(samples).permute(0, 3, 1, 2).float() / 127.5 - 1
    return tensor

def compute_lpips_diversity(images, model):
    print("Computing LPIPS...")
    scores = []
    for i in tqdm(range(len(images))):
        for j in range(i + 1, len(images)):
            d = model(images[i:i+1], images[j:j+1])
...
def compare_selected(npz_files, max_samples=100):
    ...
    for name, path in npz_files.items():
        try:
            ...
            if torch.cuda.is_available():
                images = images.cuda()
            score = compute_lpips_diversity(images, model)
            scores[name] = score
...
def plot_scores(scores):
    ...
    plt.ylabel("Avg LPIPS Diversity")
    plt.title("ODE vs SDE Diversity across Labels")
    plt.grid(axis="y")
    plt.tight_layout()
    plt.show()
